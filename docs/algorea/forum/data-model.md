...
The same DynamoDB + WebSocket notification path (`notifyUser` / `notification.new`) is also used for non-forum types such as `group_results_export.ready` and `group_results_export.failed` — see [Group results export]({{ site.baseurl }}{% link algorea/exports/group-results-export.md %}).
