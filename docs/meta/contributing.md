...
- **Operations essentials** when they belong in this corpus’s governed docs—not a second home for full operational depth already owned elsewhere. For the **Algorea** MVP-scoped hub and topic index, see **[Operations essentials](../algorea/ops.md)** (under [Algorea](../algorea/index.md)).
...
This knowledge base **must not** host a **second authoritative copy** of generated HTTP contracts. The hub’s **[What belongs elsewhere](../index.md#what-belongs-elsewhere)** states the reader-facing rule: project APIs stay **autogenerated** in project docs and are **linked** from this corpus. For Algorea, point to **[algorea-devdoc](https://france-ioi.github.io/algorea-devdoc/)** and the published **[Backend API (generated)](https://france-ioi.github.io/algorea-devdoc/api/)**—**do not** invent different URLs for those entry points.
...
- **Short illustrative mentions** in prose (for example a **single field name** or status code) when you explain **intent** or **cross-boundary behavior**, and the **authoritative** contract remains **outbound**—see **[What belongs elsewhere](../index.md#what-belongs-elsewhere)** and your page’s **References and outbound links**.
...
**Curated** rules and topic pages that should ground day-to-day consultation live under **`docs/`** (with **[docs/index.md](../index.md)** as the primary entry for **scope**). **`docs/archive/`** is the **opt-in** location for superseded narratives, deliberation, and deep history—see **[docs/archive/index.md](../archive/index.md)** and the **[Archive section](#archive-docsarchive)** in this guide. It is **not** part of the default consultation path; follow the structure contract for what counts as default path versus archive.
...
|---|--------------|-----------------|
| **Location** | **`docs/*.md`** outside **`archive/`**, discoverable via the site nav | **`docs/archive/`** tree — entry **[docs/archive/index.md](../archive/index.md)** |
...
- **Discussions** and long threads worth keeping as Markdown.
- **Deliberation** (options considered, meeting notes, long “why” threads) that would add noise on the default path.
- **Abandoned projects** — relevant information after a project or initiative is no longer active.
...
- Optional **topic subfolders** are allowed (for example **`docs/archive/algorea/topic-name.md`**) when they improve organization—**document** the pattern you use in **[docs/archive/index.md](../archive/index.md)** when introducing a new convention.
- Archive pages **must** set **`audience: archive`** (required) and **should** also set **`status: archived`** when the page is no longer evolving — per **[Default vs archive](./structure-contract.md#default-vs-archive)** so labeling makes non-default use obvious. The archive **`index.md`** itself is exempt (it uses **`audience: agent-default`** as the **Archive** section landing page in the site nav). Set **`parent: "Archive"`** so pages nest under that section in the left nav.
...
- **[docs/archive/index.md](../archive/index.md)** — purpose and browsing norms for readers.
...
2. Keep the blockquote to **one or two sentences**, name what moved or what is now canonical, and link **[docs/archive/index.md](../archive/index.md)** or a **specific** path under **`docs/archive/`** when applicable.
...
> **Note on prior guidance:** Longer deliberation and superseded drafts live under [docs/archive/](../archive/index.md). This page is the **current** default-path summary.
...
Curated Markdown under **`docs/`** **must** follow **[docs/meta/structure-contract.md](./structure-contract.md)**. That file is **normative** for how new and substantially revised pages are written so the corpus stays **machine-reliable** and consistent.
...
**Normative detail:** **[Provenance and ownership](./structure-contract.md#provenance-and-ownership)** and **[Applicable curated pages](./structure-contract.md#applicable-curated-pages)** in **`docs/meta/structure-contract.md`**. **Summary:** default-path pages outside **`docs/archive/`** that carry **cross-boundary** or **governed** narrative **must** include **`owner`** and **`last_updated`** (plus the always-required keys). The hub (**`docs/index.md`**), **meta** shells (**`docs/meta/structure-contract.md`**, **`docs/meta/default-vs-archive-split.md`**, **`docs/meta/contributing.md`**), and **`docs/meta/templates/`** scaffolds are **exempt** from that pair—see the contract. On substantive edits, advance **`date`** and **`last_updated`** together; use **`last_reviewed`** for verification against code or canonical docs without implying a synonym for **`last_updated`**.
...
**Front matter keys** are defined in **`docs/meta/structure-contract.md`**. **Synonym keys** for the same concept (for example mixing `owner` and `maintainer`) are **not** allowed without updating the contract. If you **introduce a new key**, **rename** the meaning of an existing key, or **split** one concept across multiple keys, you **must** update **`./structure-contract.md`** in the **same pull request** (or the same merged change set). Do not land new normative conventions in prose or YAML alone while the contract still describes an older schema.

## Starting from a template

**Net-new** curated pages under **`docs/*.md`** (standard topic layout) **must** start from the default scaffold in **`docs/meta/templates/topic-page.md`**, unless an exception applies—see **[When not to use the default scaffold](#when-not-to-use-the-default-scaffold)**.

1. Copy **`./templates/topic-page.md`**.
2. Save as a new file under the right **section directory** (for example **`docs/algorea/`** or **`docs/meta/`**) using a **lowercase-hyphenated** filename (for example `my-topic.md`).
3. Replace placeholders and remove HTML comments in the copy; fill **`## Purpose`**, **`## Rules and intent`**, and **`## References and outbound links`** per **[docs/meta/structure-contract.md](./structure-contract.md)**.
4. Set Just the Docs front matter (`parent`, `nav_order`, and related keys) so the page appears in the **site nav** where readers expect it.

Templates and folder overview: **[docs/meta/templates/README.md](./templates/README.md)**.
...
Keep this escape hatch **narrow**—prefer extending **`docs/meta/templates/`** and the structure contract if a new page type becomes common.
...
When you bring **Algorea-relevant** or other **in-scope** material from **legacy** sources (wikis, other repositories, exports, long deliberation threads), use the **[Legacy migration playbook](./migration.md)** as the **canonical** procedure. It walks through **slice selection**, **path mapping**, **rewrite or move** against the structure contract, **relative link repair**, and **site-nav** front matter—plus **default vs archive** placement with pointers to **[Default path vs archive](./default-vs-archive-split.md)** and the **[Archive (`docs/archive/`)](#archive-docsarchive)** rules in this guide. Substantive migrations still follow **[Substantive documentation PRs](#substantive-documentation-prs)** and the **[API documentation and OpenAPI boundary](#api-documentation-and-openapi-boundary)**.
...
- **Navigation:** new **default-path** topical pages set Just the Docs front matter so they appear in the **site nav** where expected.
...
- **Navigation** — New or **promoted** default-path topical pages appear in the **site nav** via Just the Docs front matter (`parent`, `nav_order`, and related keys).
- **Outbound link hygiene** — **[Descriptive link text](./structure-contract.md#descriptive-link-text)** for intra-repo and external links. Stable OpenAPI / project-API entry points match **[What belongs elsewhere](../index.md#what-belongs-elsewhere)** and **[API documentation and OpenAPI boundary](#api-documentation-and-openapi-boundary)**—no ad-hoc substitutes for those hubs.
