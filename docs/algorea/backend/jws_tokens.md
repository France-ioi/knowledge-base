...
Existing cross-cutting token types include the **group results token**, used to authorize the asynchronous [group results export]({{ site.baseurl }}{% link algorea/exports/group-results-export.md %}) across backend, serverless, and the worker.
