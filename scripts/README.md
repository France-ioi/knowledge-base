...
**Why validate rules:** Project rules should stay **short** and **point** to curated docs (`docs/`, `docs/meta/contributing.md`) instead of duplicating the full corpus. CI checks that each **`.cursor/rules/*.mdc`** file has parseable YAML front matter (`description`, and `alwaysApply` or `globs`), plus a non-empty body—so broken or empty rule files do not ship unnoticed.
