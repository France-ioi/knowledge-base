...
permalink: /exports/group-results-export/
---
...
Meaning: user `user_id` may obtain the results (progress + answers) of group `group_id` on the subtree of `item_ids` — the same checks as the old ZIP endpoint (`CanWatchGroupMembers` and `can_watch >= answer` on every item). See also [JWS Tokens]({{ site.baseurl }}{% link algorea/backend/jws_tokens.md %}).
...
4. **Backend worker** — accept EventBridge-shaped payloads (`detail-type` present): pipe event JSON to `AlgoreaBackend handle-event` (see [Running on AWS Lambda]({{ site.baseurl }}{% link algorea/backend/lambda.md %})). Consider `/tmp` ephemeral storage size for ZIP generation. Timeout stays 900 s; no extra IAM (plain HTTPS PUT).
...
- [JWS Tokens]({{ site.baseurl }}{% link algorea/backend/jws_tokens.md %}) — token creation and validation patterns
- [Running on AWS Lambda]({{ site.baseurl }}{% link algorea/backend/lambda.md %}) — worker / `handle-event`
- [Forum data model]({{ site.baseurl }}{% link algorea/forum/data-model.md %}) — notification storage (DynamoDB + WebSocket)
